...
<article id="post-<?php the_ID(); ?>" <?php post_class('post-wrap '. $sticky_class); ?>  >
		...
		<?php if( ( has_post_format('audio') || has_post_format('gallery') || has_post_format('video' ) || has_post_thumbnail() ) && remons_blog_show_media() == 'yes' ) : ?>
		...
		<?php if( remons_blog_show_title() == 'yes' ){
			get_template_part( 'template-parts/parts/title' );
		} ?>
		
		<?php if( remons_blog_show_excerpt() == 'yes' ){ ?>
			<div class="post-excerpt">
				<p>
					<?php echo remons_custom_text( get_the_excerpt(), 17 ); ?>
				</p>
			</div>
		<?php } ?>

		
		<?php if( remons_blog_show_readmore() == 'yes' ){
