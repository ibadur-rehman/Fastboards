...
if ( ! ( class_exists( 'woocommerce' ) && is_woocommerce() ) ) {
    if ( get_post_meta(  remons_get_current_id() ,'remons_met_show_breadcrumbs', true ) != 'no' ) {
		...
		$html = '<div id="breadcrumbs">';
			$html .= '<ul class="breadcrumb">';
		        ...
		        $html .= '<li><a href="' . esc_url( home_url('/') ) . '" title="'.esc_attr__('Home', 'remons').'">' . esc_attr__('Home', 'remons') . '</a></li> ';
		        ...
		        if ( is_category() ) {
		            ...
		            $html .=  $separator.'<li>' . esc_html__('Search results for', 'remons').' ' . get_search_query() . '</li>';
		        	...
		        	$html .= $separator.'<li>' . esc_html__('Archive by tag', 'remons').' ' . single_tag_title('', false) . '</li>';

		        } elseif ( is_author() ) {

		        	global $author;
		        	$userdata = get_userdata($author);
		        	$html .= $separator.'<li>' . esc_html__('Articles posted by', 'remons').' ' . $userdata->display_name . '</li>';

		        } elseif ( is_home() ){

		        	if( get_option('page_for_posts', true) != '0') {
				        $html .= $separator.'<li>' . get_the_title( get_option('page_for_posts', true) ).'&nbsp;' . '</li>';
				    } else {
				       $html .= $separator.'<li>' . esc_html__('Blog', 'remons').'&nbsp;' . '</li>';
				    }

		        } elseif ( is_404() ) {

		        	$html .= $separator.'<li>' . esc_html__('Page not found', 'remons') . '</li>';
